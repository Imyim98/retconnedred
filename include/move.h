...
{
    moveId = SanitizeMoveId(moveId);
    assertf(gMovesInfo[moveId].effect == EFFECT_RECOIL || gMovesInfo[moveId].effect == EFFECT_EX_SHADOW_MOVE_RECOIL || gMovesInfo[moveId].effect == EFFECT_EX_SHADOW_MOVE_RECOIL_CURRENT_HP, "not a recoil move: %S", gMovesInfo[moveId].name);
