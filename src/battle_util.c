...
bool32 IsUBWTerrainAffected(enum BattlerId battler, enum Ability ability, enum HoldEffect holdEffect, u32 fieldStatuses)
{
    return IsBattlerTerrainAffected(battler, ability, holdEffect, fieldStatuses, STATUS_FIELD_UBW);
}

bool32 IsDarknessTerrainAffected(enum BattlerId battler, enum Ability ability, enum HoldEffect holdEffect, u32 fieldStatuses)
{
    return IsBattlerTerrainAffected(battler, ability, holdEffect, fieldStatuses, STATUS_FIELD_DARKNESS_TERRAIN);
}

bool32 IsMiasmaTerrainAffected(enum BattlerId battler, enum Ability ability, enum HoldEffect holdEffect, u32 fieldStatuses)
{
...
s32 CalcCritChanceStageGen1(struct BattleContext *ctx)
{
    ...
    if (ctx->abilityDef == ABILITY_BATTLE_ARMOR || ctx->abilityDef == ABILITY_SHELL_ARMOR || ctx->abilityDef == ABILITY_GUARD_ARMOR)
    {
        if (ctx->updateFlags)
            RecordAbilityBattle(ctx->battlerDef, ctx->abilityDef);
        critChance = CRITICAL_HIT_BLOCKED;
...
void ActivateMegaEvolution(enum BattlerId battler)
{
    ...
    else if (TryBattleFormChange(battler, FORM_CHANGE_BATTLE_MEGA_EVOLUTION_LEVEL, ability))
    {
        BattleScriptPushCursorAndCallback(BattleScript_WishMegaEvolution);
    }
    else
