...
static enum MoveCanceller CancellerWeatherPrimal(struct BattleContext *ctx)
{
    ...
    if (HasWeatherEffect() && GetMovePower(ctx->currentMove) > 0)
    {
        ...
        if (moveType == TYPE_NEW_FIRE && (gBattleWeather & B_WEATHER_RAIN_PRIMAL) && (GetGenConfig(GEN_CONFIG_POWDER_RAIN) >= GEN_7 || !TryActivatePowderStatus(ctx->currentMove)))
        {
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, enum Ability ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        {
            enum Ability abilityAtk = GetBattlerAbility(gBattlerAttacker);
            if ((GetGenConfig(GEN_CONFIG_POWDER_GRASS) < GEN_6 || !IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_NEW_NATURE))
...
u32 ItemBattleEffects(enum ItemCaseId caseID, u32 battler)
{
    ...
    case ITEMEFFECT_LEFTOVERS:
        ...
        case HOLD_EFFECT_BLACK_SLUDGE:
            ...
            else if (!(IsAbilityAndRecord(battler, GetBattlerAbility(battler), ABILITY_MAGIC_GUARD) || IsAbilityAndRecord(battler, GetBattlerAbility(battler), ABILITY_FANTASY_BREAKER)))
            {
                gBattleScripting.battler = battler;
                gBattleStruct->moveDamage[battler] = GetNonDynamaxMaxHP(battler) / 8;
                if (gBattleStruct->moveDamage[battler] == 0)
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    if (gBattleMons[battlerAtk].volatiles.charge && moveType == TYPE_NEW_ELECTRIC)
        modifier = uq4_12_multiply(modifier, UQ_4_12(2.0));
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct DamageContext *ctx, uq4_12_t modifier)
{
    ...
    else if (ctx->moveType == TYPE_NEW_EARTH && !IsBattlerGroundedInverseCheck(ctx->battlerDef, ctx->abilityDef, ctx->holdEffectDef, INVERSE_BATTLE) && !(MoveIgnoresTypeIfFlyingAndUngrounded(ctx->move)))
    {
        modifier = UQ_4_12(0.0);
...
bool32 IsBattlerAffectedByHazards(u32 battler, bool32 toxicSpikes)
{
    bool32 ret = TRUE;
    enum ItemHoldEffect holdEffect = GetBattlerHoldEffect(battler);
    if (toxicSpikes && holdEffect == HOLD_EFFECT_HEAVY_DUTY_BOOTS && !IS_BATTLER_OF_TYPE(battler, TYPE_NEW_MIASMA))
    {
        ret = FALSE;
