...
bool32 CanAbilityAbsorbMove(struct BattleContext *ctx)
{
    ...
    case ABILITY_VOLT_ABSORB:
        if (ctx->moveType == TYPE_NEW_ELECTRIC)
            battleScript = AbsorbedByDrainHpAbility(ctx->battlerDef);
        break;
    case ABILITY_WATER_ABSORB:
    case ABILITY_DRY_SKIN:
        if (ctx->moveType == TYPE_NEW_WATER)
            battleScript = AbsorbedByDrainHpAbility(ctx->battlerDef);
        break;
    ...
    case ABILITY_LIGHTNING_ROD:
        if (GetConfig(CONFIG_REDIRECT_ABILITY_IMMUNITY) >= GEN_5 && ctx->moveType == TYPE_NEW_ELECTRIC)
            battleScript = AbsorbedByStatIncreaseAbility(ctx->battlerDef, ctx->abilityDef, STAT_SPATK, 1);
        break;
    case ABILITY_STORM_DRAIN:
        if (GetConfig(CONFIG_REDIRECT_ABILITY_IMMUNITY) >= GEN_5 && ctx->moveType == TYPE_NEW_WATER)
            battleScript = AbsorbedByStatIncreaseAbility(ctx->battlerDef, ctx->abilityDef, STAT_SPATK, 1);
        break;
    case ABILITY_SAP_SIPPER:
        if (ctx->moveType == TYPE_NEW_NATURE)
            battleScript = AbsorbedByStatIncreaseAbility(ctx->battlerDef, ctx->abilityDef, STAT_ATK, 1);
        break;
    case ABILITY_WELL_BAKED_BODY:
        if (ctx->moveType == TYPE_NEW_FIRE)
...
bool32 CanMonParticipateInSkyBattle(struct Pokemon *mon)
{
    ...
    bool32 isFlyingType = GetSpeciesType(species, 0) == TYPE_NEW_FLYING || GetSpeciesType(species, 1) == TYPE_NEW_FLYING;
...
bool32 IsAffectedByPowderMove(u32 battler, enum Ability ability, enum HoldEffect holdEffect)
{
    if (GetConfig(CONFIG_POWDER_OVERCOAT) >= GEN_6 && (ability == ABILITY_OVERCOAT || ability == ABILITY_WIDE_HAT))
        return FALSE;
    if (GetConfig(CONFIG_POWDER_GRASS) >= GEN_6 && IS_BATTLER_OF_TYPE(battler, TYPE_NEW_NATURE))
        return FALSE;
