...
static enum MoveCanceler CancelerWeatherPrimal(struct BattleContext *ctx)
{
    ...
    if (HasWeatherEffect() && GetMovePower(ctx->move) > 0)
    {
        ...
        if (moveType == TYPE_NEW_FIRE && (gBattleWeather & B_WEATHER_RAIN_PRIMAL) && (GetGenConfig(GEN_CONFIG_POWDER_STATUS_HEAVY_RAIN) >= GEN_7 || !TryActivatePowderStatus(ctx->move)))
        {
