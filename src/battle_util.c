...
bool32 CanAbilityAbsorbMove(struct BattleContext *ctx)
{
    ...
    case ABILITY_LIGHTNING_ROD:
        if (GetConfig(B_REDIRECT_ABILITY_IMMUNITY) >= GEN_5 && ctx->moveType == TYPE_NEW_ELECTRIC)
            battleScript = AbsorbedByStatIncreaseAbility(ctx->battlerDef, ctx->abilityDef, STAT_SPATK, 1);
        break;
    case ABILITY_STORM_DRAIN:
        if (GetConfig(B_REDIRECT_ABILITY_IMMUNITY) >= GEN_5 && ctx->moveType == TYPE_NEW_WATER)
            battleScript = AbsorbedByStatIncreaseAbility(ctx->battlerDef, ctx->abilityDef, STAT_SPATK, 1);
        break;
...
{
    if (GetConfig(B_GHOSTS_ESCAPE) >= GEN_6 && IS_BATTLER_OF_TYPE(battler, TYPE_NEW_NETHER))
        return 0;
...
bool32 CanSetNonVolatileStatus(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability abilityAtk, enum Ability abilityDef, enum MoveEffect effect, enum ResultOption option)
{
    ...
    case MOVE_EFFECT_PARALYSIS:
        ...
        else if (GetConfig(B_PARALYZE_ELECTRIC) >= GEN_6 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_ELECTRIC))
        {
            battleScript = BattleScript_NotAffected;
        }
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct BattleContext *ctx)
{
    ...
    case ABILITY_NORMALIZE:
        if (moveType == TYPE_NEW_ILLUSION && gBattleStruct->battlerState[battlerAtk].ateBoost && GetConfig(B_ATE_MULTIPLIER) >= GEN_7)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;
...
static inline u32 CalcDefenseStat(struct BattleContext *ctx)
{
    ...
    if (GetConfig(B_SANDSTORM_SPDEF_BOOST) >= GEN_4 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_EARTH) && IsBattlerWeatherAffected(battlerDef, B_WEATHER_SANDSTORM) && !usesDefStat)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
...
static inline uq4_12_t GetDefenderAbilitiesModifier(struct BattleContext *ctx)
{
    ...
    case ABILITY_PRISM_ARMOR:
        ...
        break;
    case ABILITY_FLUFFY:
        if (ctx->moveType == TYPE_NEW_FIRE && !IsMoveMakingContact(ctx->battlerAtk, ctx->battlerDef, ctx->abilityAtk, ctx->holdEffectAtk, ctx->move))
        {
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct BattleContext *ctx, uq4_12_t modifier)
{
    ...
    if ((((ctx->abilityDef == ABILITY_WONDER_GUARD || ctx->abilityDef == ABILITY_PLAY_GHOST) && modifier <= UQ_4_12(1.0) && !isPresentHealing)
...
{
    if (GetConfig(B_POWDER_OVERCOAT) >= GEN_6 && (ability == ABILITY_OVERCOAT || ability == ABILITY_WIDE_HAT))
        return FALSE;
    if (GetConfig(B_POWDER_GRASS) >= GEN_6 && IS_BATTLER_OF_TYPE(battler, TYPE_NEW_NATURE))
        return FALSE;
