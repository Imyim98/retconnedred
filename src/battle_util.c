...
bool32 TryRunFromBattle(u32 battler)
{
    ...
    else if (GetConfig(CONFIG_GHOSTS_ESCAPE) >= GEN_6 && IS_BATTLER_OF_TYPE(battler, TYPE_NEW_NETHER))
    {
        effect++;
    }
...
static enum MoveCanceler CancelerParalyzed(struct BattleContext *ctx)
{
    if (gBattleMons[ctx->battlerAtk].status1 & STATUS1_PARALYSIS
        && !(B_MAGIC_GUARD == GEN_4 && (IsAbilityAndRecord(ctx->battlerAtk, ctx->abilityAtk, ABILITY_MAGIC_GUARD) || IsAbilityAndRecord(ctx->battlerAtk, ctx->abilityAtk, ABILITY_FANTASY_BREAKER)))
...
static enum MoveCanceler CancelerPowderStatus(struct BattleContext *ctx)
{
    ...
    {
        if (!(IsAbilityAndRecord(ctx->battlerAtk, ctx->abilityAtk, ABILITY_MAGIC_GUARD) || IsAbilityAndRecord(ctx->battlerAtk, ctx->abilityAtk, ABILITY_FANTASY_BREAKER)))
...
static enum MoveCanceler CancelerPriorityBlock(struct BattleContext *ctx)
{
    ...
    if (IsDazzlingAbility(ctx->abilityDef) && !IsAbilityOnField(ABILITY_TIME_JACKER))
    {
...
bool32 ChangeTypeBasedOnTerrain(u32 battler)
{
    ...
    if (gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN)
        battlerType = TYPE_NEW_ELECTRIC;
    else if (gFieldStatuses & STATUS_FIELD_GRASSY_TERRAIN)
        battlerType = TYPE_NEW_NATURE;
    else if (gFieldStatuses & STATUS_FIELD_MISTY_TERRAIN)
        battlerType = TYPE_NEW_DIVINE;
...
bool32 CanAbilityAbsorbMove(u32 battlerAtk, u32 battlerDef, enum Ability abilityDef, enum Move move, enum Type moveType, enum FunctionCallOption option)
{
    ...
    case ABILITY_VOLT_ABSORB:
        if (moveType == TYPE_NEW_ELECTRIC && target != TARGET_ALL_BATTLERS && target != TARGET_FIELD && gCurrentMove != MOVE_BINARY_CRUSHER)
    ...
    case ABILITY_LIGHTNING_ROD:
        if (GetConfig(CONFIG_REDIRECT_ABILITY_IMMUNITY) >= GEN_5 && moveType == TYPE_NEW_ELECTRIC && GetBattlerMoveTargetType(battlerAtk, move) != TARGET_ALL_BATTLERS && gCurrentMove != MOVE_BINARY_CRUSHER)
        {
            effect = MOVE_ABSORBED_BY_STAT_INCREASE_ABILITY;
            statId = STAT_SPATK;
        }
    ...
    case ABILITY_STORM_DRAIN:
        if (GetConfig(CONFIG_REDIRECT_ABILITY_IMMUNITY) >= GEN_5 && moveType == TYPE_NEW_WATER)
        {
            effect = MOVE_ABSORBED_BY_STAT_INCREASE_ABILITY;
            statId = STAT_SPATK;
        }
    ...
    case ABILITY_FLASH_FIRE:
        if (moveType == TYPE_NEW_FIRE && (B_FLASH_FIRE_FROZEN >= GEN_5 || !(gBattleMons[battlerDef].status1 & STATUS1_FREEZE)))
...
u32 AbilityBattleEffects(enum AbilityEffect caseID, u32 battler, enum Ability ability, enum Move move, bool32 shouldAbilityTrigger)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_BLANK_CARD:
            {
                struct Pokemon *party, *partnerMon, *mon;
                u32 id;
                enum Ability blankCardTarget;
                ...
                blankCardTarget = GetMonAbility(&gPlayerParty[id]), effect++;
                ...
                gBattleStruct->tracedAbility[gBattlerAttacker] = gLastUsedAbility = blankCardTarget;
                PREPARE_MON_NICK_WITH_PREFIX_LOWER_BUFFER(gBattleTextBuff1, battler, gBattlerPartyIndexes[battler])
                PREPARE_ABILITY_BUFFER(gBattleTextBuff2, gLastUsedAbility)
            }
            break;
        ...
        case ABILITY_WORLD_END_GARDEN:
            if (shouldAbilityTrigger)
            {
                ...
                gSideStatuses[targetSide] &= ~SIDE_STATUS_MIST;
                gSideTimers[targetSide].mistTimer = 0;

                gSideStatuses[targetSide] &= ~SIDE_STATUS_TAILWIND;
                gSideTimers[targetSide].tailwindTimer = 0;

                gSideStatuses[targetSide] &= ~SIDE_STATUS_AURORA_VEIL;
                gSideTimers[targetSide].auroraVeilTimer = 0;
                ...
                gSideStatuses[targetSide] &= ~SIDE_STATUS_RAINBOW;
                gSideTimers[targetSide].rainbowTimer = 0;

                gSideStatuses[targetSide] |= SIDE_STATUS_SEA_OF_FIRE;
                gSideTimers[targetSide].seaOfFireTimer = 4;
        ...
        if (IsBattlerAlive(battler))
        {
            ...
            case ABILITY_POWER_LEAK:
                if (CompareStat(battler, STAT_SPATK, 0, CMP_GREATER_THAN, gLastUsedAbility) && gBattleStruct->battlerState[battler].isFirstTurn != 2)
                {
                    SaveBattlerAttacker(gBattlerAttacker);
                    SET_STATCHANGER(STAT_SPATK, 1, TRUE);
                    BattleScriptExecute(BattleScript_AttackerAbilityStatDropEnd2);
                    gBattleScripting.battler = battler;
                    effect++;
                }
                break;
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_RATTLED:
            if (IsBattlerTurnDamaged(battler)
             && IsBattlerAlive(battler)
             && (moveType == TYPE_NEW_DARK || moveType == TYPE_NEW_BEAST || moveType == TYPE_NEW_NETHER)
        ...
        case ABILITY_MIRROR_WALL:
             ...
             && IsBattleMoveSpecial(move))
            {
                SetPassiveDamageAmount(gBattlerAttacker, GetNonDynamaxMaxHP(gBattlerAttacker) / (B_ROUGH_SKIN_DMG >= GEN_4 ? 8 : 16));
                PREPARE_ABILITY_BUFFER(gBattleTextBuff1, gLastUsedAbility);
                BattleScriptCall(BattleScript_MirrorWallActivates);
                effect++;
            }
            break;
        ...
        case ABILITY_STEAM_ENGINE:
            if (IsBattlerTurnDamaged(gBattlerTarget)
             && IsBattlerAlive(battler)
             && CompareStat(battler, STAT_SPEED, MAX_STAT_STAGE, CMP_LESS_THAN, gLastUsedAbility)
             && (moveType == TYPE_NEW_FIRE || moveType == TYPE_NEW_WATER))
            {
                gEffectBattler = gBattlerAbility = battler;
             ...
             && !gSpecialStatuses[battler].gemBoost) // In base game, gems are consumed after magician would activate.
            {
                ...
                for (u32 battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
                {
                    if (gBattleMons[battlerDef].item != ITEM_NONE
                     ...
                     && (!(GetBattlerAbility(battlerDef) == ABILITY_STICKY_HOLD || GetBattlerAbility(battlerDef) == ABILITY_COLLECTOR) || !IsBattlerAlive(battlerDef)))
                    {
...
{
    if (GetConfig(CONFIG_GHOSTS_ESCAPE) >= GEN_6 && IS_BATTLER_OF_TYPE(battler, TYPE_NEW_NETHER))
        return 0;
...
bool32 IsUBWTerrainAffected(u32 battler, enum Ability ability, enum HoldEffect holdEffect, u32 fieldStatuses)
{
    return IsBattlerTerrainAffected(battler, ability, holdEffect, fieldStatuses, STATUS_FIELD_UBW);
}

bool32 IsDarknessTerrainAffected(u32 battler, enum Ability ability, enum HoldEffect holdEffect, u32 fieldStatuses)
{
    return IsBattlerTerrainAffected(battler, ability, holdEffect, fieldStatuses, STATUS_FIELD_DARKNESS_TERRAIN);
}

bool32 IsMiasmaTerrainAffected(u32 battler, enum Ability ability, enum HoldEffect holdEffect, u32 fieldStatuses)
{
...
bool32 CanSetNonVolatileStatus(u32 battlerAtk, u32 battlerDef, enum Ability abilityAtk, enum Ability abilityDef, enum MoveEffect effect, enum FunctionCallOption option)
{
    ...
    case MOVE_EFFECT_PARALYSIS:
        ...
        else if (GetConfig(CONFIG_PARALYZE_ELECTRIC) >= GEN_6 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_ELECTRIC))
        {
            battleScript = BattleScript_NotAffected;
        }
...
bool32 HasEnoughHpToEatBerry(u32 battler, enum Ability ability, u32 hpFraction, u32 itemId)
{
    ...
    if (IsAbilityAndRecord(battler, GetBattlerAbility(battler), ABILITY_GOURMAND))
    {
        switch (itemId)
        {
...
u32 GetBattleMoveTarget(enum Move move, enum MoveTarget moveTarget)
{
    ...
    case TARGET_OPPONENT:
        ...
        else
        {
            ...
            if (moveType == TYPE_NEW_ELECTRIC && GetBattlerAbility(targetBattler) != ABILITY_LIGHTNING_ROD)
            {
            ...
            else if (moveType == TYPE_NEW_WATER && GetBattlerAbility(targetBattler) != ABILITY_STORM_DRAIN)
            {
                if (B_REDIRECT_ABILITY_ALLIES >= GEN_4)
...
static bool32 IsBattlerGroundedInverseCheck(u32 battler, enum Ability ability, enum HoldEffect holdEffect, enum InverseBattleCheck checkInverse, bool32 isAnticipation)
{
    ...
    if (ability == ABILITY_LEVITATE)
        return FALSE;
    if (IS_BATTLER_OF_TYPE(battler, TYPE_NEW_FLYING) && (checkInverse != INVERSE_BATTLE || !FlagGet(B_FLAG_INVERSE_BATTLE)))
        return FALSE;
    return TRUE;
}
...
{
    [ITEM_TO_BERRY(ITEM_CHERI_BERRY)] = {TYPE_NEW_FIRE, 80},
    [ITEM_TO_BERRY(ITEM_CHESTO_BERRY)] = {TYPE_NEW_WATER, 80},
    [ITEM_TO_BERRY(ITEM_PECHA_BERRY)] = {TYPE_NEW_WIND, 80},
    [ITEM_TO_BERRY(ITEM_RAWST_BERRY)] = {TYPE_NEW_NATURE, 80},
    [ITEM_TO_BERRY(ITEM_ASPEAR_BERRY)] = {TYPE_NEW_ICE, 80},
    [ITEM_TO_BERRY(ITEM_LEPPA_BERRY)] = {TYPE_NEW_DREAM, 80},
    [ITEM_TO_BERRY(ITEM_ORAN_BERRY)] = {TYPE_NEW_MIASMA, 80},
    [ITEM_TO_BERRY(ITEM_PERSIM_BERRY)] = {TYPE_NEW_EARTH, 80},
    [ITEM_TO_BERRY(ITEM_LUM_BERRY)] = {TYPE_NEW_FLYING, 80},
    [ITEM_TO_BERRY(ITEM_SITRUS_BERRY)] = {TYPE_NEW_REASON, 80},
    [ITEM_TO_BERRY(ITEM_FIGY_BERRY)] = {TYPE_NEW_HEART, 80},
    [ITEM_TO_BERRY(ITEM_WIKI_BERRY)] = {TYPE_NEW_BEAST, 80},
    ...
    [ITEM_TO_BERRY(ITEM_RINDO_BERRY)] = {TYPE_NEW_NATURE, 80},
    [ITEM_TO_BERRY(ITEM_YACHE_BERRY)] = {TYPE_NEW_ICE, 80},
    [ITEM_TO_BERRY(ITEM_CHOPLE_BERRY)] = {TYPE_NEW_DREAM, 80},
    [ITEM_TO_BERRY(ITEM_KEBIA_BERRY)] = {TYPE_NEW_MIASMA, 80},
    [ITEM_TO_BERRY(ITEM_SHUCA_BERRY)] = {TYPE_NEW_EARTH, 80},
    [ITEM_TO_BERRY(ITEM_COBA_BERRY)] = {TYPE_NEW_FLYING, 80},
    [ITEM_TO_BERRY(ITEM_PAYAPA_BERRY)] = {TYPE_NEW_REASON, 80},
    [ITEM_TO_BERRY(ITEM_TANGA_BERRY)] = {TYPE_NEW_HEART, 80},
    [ITEM_TO_BERRY(ITEM_CHARTI_BERRY)] = {TYPE_NEW_BEAST, 80},
    ...
    [ITEM_TO_BERRY(ITEM_NANAB_BERRY)] = {TYPE_NEW_WATER, 90},
    [ITEM_TO_BERRY(ITEM_WEPEAR_BERRY)] = {TYPE_NEW_WIND, 90},
    [ITEM_TO_BERRY(ITEM_PINAP_BERRY)] = {TYPE_NEW_NATURE, 90},
    [ITEM_TO_BERRY(ITEM_POMEG_BERRY)] = {TYPE_NEW_ICE, 90},
    [ITEM_TO_BERRY(ITEM_KELPSY_BERRY)] = {TYPE_NEW_DREAM, 90},
    [ITEM_TO_BERRY(ITEM_QUALOT_BERRY)] = {TYPE_NEW_MIASMA, 90},
    [ITEM_TO_BERRY(ITEM_HONDEW_BERRY)] = {TYPE_NEW_EARTH, 90},
    [ITEM_TO_BERRY(ITEM_GREPA_BERRY)] = {TYPE_NEW_FLYING, 90},
    [ITEM_TO_BERRY(ITEM_TAMATO_BERRY)] = {TYPE_NEW_REASON, 90},
    [ITEM_TO_BERRY(ITEM_CORNN_BERRY)] = {TYPE_NEW_HEART, 90},
    [ITEM_TO_BERRY(ITEM_MAGOST_BERRY)] = {TYPE_NEW_BEAST, 90},
    ...
    [ITEM_TO_BERRY(ITEM_LIECHI_BERRY)] = {TYPE_NEW_NATURE, 100},
    [ITEM_TO_BERRY(ITEM_GANLON_BERRY)] = {TYPE_NEW_ICE, 100},
    [ITEM_TO_BERRY(ITEM_SALAC_BERRY)] = {TYPE_NEW_DREAM, 100},
    [ITEM_TO_BERRY(ITEM_PETAYA_BERRY)] = {TYPE_NEW_MIASMA, 100},
    [ITEM_TO_BERRY(ITEM_APICOT_BERRY)] = {TYPE_NEW_EARTH, 100},
    [ITEM_TO_BERRY(ITEM_LANSAT_BERRY)] = {TYPE_NEW_FLYING, 100},
    [ITEM_TO_BERRY(ITEM_STARF_BERRY)] = {TYPE_NEW_REASON, 100},
    [ITEM_TO_BERRY(ITEM_ENIGMA_BERRY)] = {TYPE_NEW_HEART, 100},
    [ITEM_TO_BERRY(ITEM_MICLE_BERRY)] = {TYPE_NEW_BEAST, 100},
    [ITEM_TO_BERRY(ITEM_CUSTAP_BERRY)] = {TYPE_NEW_NETHER, 100},
    [ITEM_TO_BERRY(ITEM_JABOCA_BERRY)] = {TYPE_NEW_DIVINE, 100},
    [ITEM_TO_BERRY(ITEM_ROWAP_BERRY)] = {TYPE_NEW_DARK, 100},
    [ITEM_TO_BERRY(ITEM_KEE_BERRY)] = {TYPE_NEW_ELECTRIC, 100},
    [ITEM_TO_BERRY(ITEM_MARANGA_BERRY)] = {TYPE_NEW_DARK, 100},
};
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct BattleContext *ctx)
{
    ...
    case EFFECT_SOLAR_BEAM:
        if ((GetConfig(CONFIG_SANDSTORM_SOLAR_BEAM) >= GEN_3 && IsBattlerWeatherAffected(battlerAtk, B_WEATHER_LOW_LIGHT))
            || IsBattlerWeatherAffected(battlerAtk, (B_WEATHER_RAIN | B_WEATHER_ICY_ANY | B_WEATHER_FOG | B_WEATHER_EX_SHADOW_SKY))) // Excludes Sandstorm
    ...
    if (moveType == TYPE_NEW_ELECTRIC && gBattleMons[battlerAtk].volatiles.chargeTimer > 0)
        modifier = uq4_12_multiply(modifier, UQ_4_12(2.0));
    if (GetMoveEffect(ctx->chosenMove) == EFFECT_ME_FIRST)
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
    if (IsGrassyTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, ctx->fieldStatuses) && moveType == TYPE_NEW_NATURE)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsMistyTerrainAffected(battlerDef, ctx->abilityDef, ctx->holdEffectDef, ctx->fieldStatuses) && moveType == TYPE_NEW_DARK)
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
    if (IsElectricTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, ctx->fieldStatuses) && moveType == TYPE_NEW_ELECTRIC)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsPsychicTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, ctx->fieldStatuses) && moveType == TYPE_NEW_REASON)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));

    if (IsUBWTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, ctx->fieldStatuses) && moveType == TYPE_NEW_METAL)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsUBWTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, ctx->fieldStatuses) && moveType == TYPE_NEW_MIASMA)
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
    if (IsDarknessTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, ctx->fieldStatuses) && moveType == TYPE_NEW_DARK)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsDarknessTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, ctx->fieldStatuses) && moveType == TYPE_NEW_DIVINE)
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
    if (IsMiasmaTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, ctx->fieldStatuses) && moveType == TYPE_NEW_MIASMA)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    ...
    case ABILITY_PIXILATE:
        if (moveType == TYPE_NEW_HEART && gBattleStruct->battlerState[battlerAtk].ateBoost)
            modifier = uq4_12_multiply(modifier, UQ_4_12(GetConfig(CONFIG_ATE_MULTIPLIER) >= GEN_7 ? 1.2 : 1.3));
        break;
    case ABILITY_GALVANIZE:
        if (moveType == TYPE_NEW_ELECTRIC && gBattleStruct->battlerState[battlerAtk].ateBoost)
            modifier = uq4_12_multiply(modifier, UQ_4_12(GetConfig(CONFIG_ATE_MULTIPLIER) >= GEN_7 ? 1.2 : 1.3));
        break;
    case ABILITY_REFRIGERATE:
        if (moveType == TYPE_NEW_ICE && gBattleStruct->battlerState[battlerAtk].ateBoost)
            modifier = uq4_12_multiply(modifier, UQ_4_12(GetConfig(CONFIG_ATE_MULTIPLIER) >= GEN_7 ? 1.2 : 1.3));
        break;
    case ABILITY_AERILATE:
        if (moveType == TYPE_NEW_FLYING && gBattleStruct->battlerState[battlerAtk].ateBoost)
            modifier = uq4_12_multiply(modifier, UQ_4_12(GetConfig(CONFIG_ATE_MULTIPLIER) >= GEN_7 ? 1.2 : 1.3));
        break;
    case ABILITY_NORMALIZE:
        if (moveType == TYPE_NEW_ILLUSION && gBattleStruct->battlerState[battlerAtk].ateBoost && GetConfig(CONFIG_ATE_MULTIPLIER) >= GEN_7)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;
...
static inline u32 CalcAttackStat(struct BattleContext *ctx)
{
    ...
    else if (moveEffect == EFFECT_ILLUSIONARY_DOMINATION)
    {
    ...
    case ABILITY_DEFEATIST:
        ...
        break;
    case ABILITY_FLASH_FIRE:
        if (moveType == TYPE_NEW_FIRE && gBattleMons[battlerAtk].volatiles.flashFireBoosted)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_SWARM:
        if (moveType == TYPE_NEW_BEAST && gBattleMons[battlerAtk].hp <= (gBattleMons[battlerAtk].maxHP / 2))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_TORRENT:
        if (moveType == TYPE_NEW_WATER && gBattleMons[battlerAtk].hp <= (gBattleMons[battlerAtk].maxHP / 2))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_BLAZE:
        if (moveType == TYPE_NEW_FIRE && gBattleMons[battlerAtk].hp <= (gBattleMons[battlerAtk].maxHP / 2))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_OVERGROW:
        if (moveType == TYPE_NEW_NATURE && gBattleMons[battlerAtk].hp <= (gBattleMons[battlerAtk].maxHP / 2))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_INNER_POWER:
        if ((moveType == gBattleMons[battlerAtk].types[0]
          || moveType == gBattleMons[battlerAtk].types[1]
          || moveType == gBattleMons[battlerAtk].types[2])
    ...
    case ABILITY_THICK_FAT:
        if (moveType == TYPE_NEW_FIRE || moveType == TYPE_NEW_ICE)
        {
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.5));
            if (ctx->updateFlags)
...
static inline u32 CalcDefenseStat(struct BattleContext *ctx)
{
    ...
    case ABILITY_SPRING_CHARM:
        ...
        break;
    case ABILITY_WINTER_GIFT:
        if ((IsBattlerWeatherAffected(battlerDef, B_WEATHER_SNOW) || IsBattlerWeatherAffected(battlerDef, B_WEATHER_HAIL)) && !usesDefStat)
        {
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
            if (ctx->updateFlags)
                RecordAbilityBattle(battlerDef, ABILITY_WINTER_GIFT);
        }
        break;
    ...
    if (GetConfig(CONFIG_SANDSTORM_SPDEF_BOOST) >= GEN_4 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_EARTH) && IsBattlerWeatherAffected(battlerDef, B_WEATHER_SANDSTORM) && !usesDefStat)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
    // snow def boost for ice types
    if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_ICE) && IsBattlerWeatherAffected(battlerDef, B_WEATHER_SNOW) && usesDefStat)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
...
static uq4_12_t GetWeatherDamageModifier(struct BattleContext *ctx)
{
    ...
    {
        if (ctx->moveType != TYPE_NEW_FIRE && ctx->moveType != TYPE_NEW_WATER)
            return UQ_4_12(1.0);
        return (ctx->moveType == TYPE_NEW_FIRE) ? UQ_4_12(0.5) : UQ_4_12(1.5);
    }
    if (ctx->weather & B_WEATHER_SUN)
    {
        if (ctx->moveType != TYPE_NEW_FIRE && ctx->moveType != TYPE_NEW_WATER)
            return UQ_4_12(1.0);
        return (ctx->moveType == TYPE_NEW_WATER) ? UQ_4_12(0.5) : UQ_4_12(1.5);
    }
    return UQ_4_12(1.0);
}
...
static inline uq4_12_t GetDefenderAbilitiesModifier(struct BattleContext *ctx)
{
    ...
    case ABILITY_PRISM_ARMOR:
        ...
        break;
    case ABILITY_FLUFFY:
        if (ctx->moveType == TYPE_NEW_FIRE && !IsMoveMakingContact(ctx->battlerAtk, ctx->battlerDef, ABILITY_NONE, ctx->holdEffectAtk, ctx->move))
        {
...
static inline uq4_12_t GetDefenderItemsModifier(struct BattleContext *ctx)
{
    ...
    case HOLD_EFFECT_RESIST_BERRY:
        if (IsUnnerveBlocked(ctx->battlerDef, gBattleMons[ctx->battlerDef].item))
            return UQ_4_12(1.0);
        if (ctx->moveType == GetBattlerHoldEffectParam(ctx->battlerDef) && (ctx->moveType == TYPE_NEW_ILLUSION || ctx->typeEffectivenessModifier >= UQ_4_12(2.0)))
        {
            if (ctx->updateFlags)
...
s32 CalcCritChanceStage(struct BattleContext *ctx)
{
    ...
    if (critChance != CRITICAL_HIT_BLOCKED && (ctx->abilityDef == ABILITY_BATTLE_ARMOR || ctx->abilityDef == ABILITY_SHELL_ARMOR || ctx->abilityDef == ABILITY_GUARD_ARMOR))
    {
...
s32 CalcCritChanceStageGen1(struct BattleContext *ctx)
{
    ...
    if (gSideStatuses[ctx->battlerDef] & SIDE_STATUS_LUCKY_CHANT)
        critChance = CRITICAL_HIT_BLOCKED;
    else if (ctx->abilityDef == ABILITY_BATTLE_ARMOR || ctx->abilityDef == ABILITY_SHELL_ARMOR || ctx->abilityDef == ABILITY_GUARD_ARMOR)
    {
        if (ctx->updateFlags)
            RecordAbilityBattle(ctx->battlerDef, ctx->abilityDef);
        critChance = CRITICAL_HIT_BLOCKED;
...
static inline void MulByTypeEffectiveness(struct BattleContext *ctx, uq4_12_t *modifier, enum Type defType)
{
    ...
    else if ((ctx->moveType == TYPE_NEW_DREAM || ctx->moveType == TYPE_NEW_ILLUSION) && defType == TYPE_NEW_NETHER && gBattleMons[ctx->battlerDef].volatiles.foresight && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
    }
    else if ((ctx->moveType == TYPE_NEW_DREAM || ctx->moveType == TYPE_NEW_ILLUSION) && defType == TYPE_NEW_NETHER
    ...
    if (ctx->moveType == TYPE_NEW_MIASMA && defType == TYPE_NEW_STEEL && (gFieldStatuses & STATUS_FIELD_MIASMA_TERRAIN) && mod == UQ_4_12(0.0))
        mod = UQ_4_12(1.0);
    if (ctx->moveType == TYPE_NEW_EARTH && defType == TYPE_NEW_FLYING && IsBattlerGrounded(ctx->battlerDef, ctx->abilityDef, ctx->holdEffectDef) && mod == UQ_4_12(0.0))
        mod = UQ_4_12(1.0);
    ...
    {
        if (defType == TYPE_NEW_WIND && mod >= UQ_4_12(2.0))
            mod = UQ_4_12(1.0);
    }
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct BattleContext *ctx, uq4_12_t modifier)
{
    ...
    if (ctx->moveType == TYPE_NEW_FIRE && gBattleMons[ctx->battlerDef].volatiles.tarShot)
        modifier = uq4_12_multiply(modifier, UQ_4_12(2.0));
    ...
    else if (ctx->moveType == TYPE_NEW_EARTH && !IsBattlerGroundedInverseCheck(ctx->battlerDef, ctx->abilityDef, ctx->holdEffectDef, INVERSE_BATTLE, ctx->isAnticipation) && !(MoveIgnoresTypeIfFlyingAndUngrounded(ctx->move)))
    {
        modifier = UQ_4_12(0.0);
    ...
    else if (GetConfig(CONFIG_SHEER_COLD_IMMUNITY) >= GEN_7 && GetMoveEffect(ctx->move) == EFFECT_SHEER_COLD && IS_BATTLER_OF_TYPE(ctx->battlerDef, TYPE_NEW_ICE))
    {
        modifier = UQ_4_12(0.0);
    }
    ...
    if ((((ctx->abilityDef == ABILITY_WONDER_GUARD || ctx->abilityDef == ABILITY_PLAY_GHOST) && modifier <= UQ_4_12(1.0))
...
uq4_12_t GetOverworldTypeEffectiveness(struct Pokemon *mon, enum Type moveType)
{
    ...
    if ((modifier <= UQ_4_12(1.0) && (abilityDef == ABILITY_WONDER_GUARD || abilityDef == ABILITY_PLAY_GHOST))
...
bool32 IsBattlerAffectedByHazards(u32 battler, enum HoldEffect holdEffect, bool32 toxicSpikes)
{
    ...
    else if (toxicSpikes && holdEffect == HOLD_EFFECT_HEAVY_DUTY_BOOTS && !IS_BATTLER_OF_TYPE(battler, TYPE_NEW_MIASMA))
    {
        ret = FALSE;
...
bool32 BlocksPrankster(enum Move move, u32 battlerPrankster, u32 battlerDef, bool32 checkTarget)
{
    ...
    if (checkTarget && GetBattlerMoveTargetType(battlerPrankster, move) == TARGET_OPPONENTS_FIELD)
        return FALSE;
    if (!IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_DARK))
        return FALSE;
...
bool8 CanMonParticipateInSkyBattle(struct Pokemon *mon)
{
    ...
    bool8 isFlyingType = GetSpeciesType(species, 0) == TYPE_NEW_FLYING || GetSpeciesType(species, 1) == TYPE_NEW_FLYING;
...
void GetBattlerTypes(u32 battler, bool32 ignoreTera, enum Type types[static 3])
{
    ...
    {
        if (types[0] == TYPE_NEW_FLYING && types[1] == TYPE_NEW_FLYING)
            types[0] = types[1] = B_ROOST_PURE_FLYING >= GEN_5 ? TYPE_NEW_ILLUSION : TYPE_MYSTERY;
        else if (types[0] == TYPE_NEW_FLYING)
            types[0] = TYPE_MYSTERY;
        else if (types[1] == TYPE_NEW_FLYING)
...
bool32 CanMoveSkipAccuracyCalc(u32 battlerAtk, u32 battlerDef, enum Ability abilityAtk, enum Ability abilityDef, enum Move move, enum FunctionCallOption option)
{
    ...
    if ((gBattleMons[battlerDef].volatiles.lockOn && gBattleMons[battlerDef].volatiles.battlerWithSureHit == battlerAtk)
     || (GetConfig(CONFIG_TOXIC_NEVER_MISS) >= GEN_6 && nonVolatileStatus == MOVE_EFFECT_TOXIC && IS_BATTLER_OF_TYPE(battlerAtk, TYPE_NEW_MIASMA))
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, enum Move move, enum Ability atkAbility, enum Ability defAbility, enum HoldEffect atkHoldEffect, enum HoldEffect defHoldEffect)
{
    ...
    if (atkAbility == ABILITY_UNAWARE || atkAbility == ABILITY_KEEN_EYE || atkAbility == ABILITY_MINDS_EYE
            || (GetConfig(CONFIG_ILLUMINATE_EFFECT) >= GEN_9 && (atkAbility == ABILITY_ILLUMINATE || atkAbility == ABILITY_DIVA)))
...
{
    if ((GetConfig(CONFIG_POWDER_OVERCOAT) >= GEN_6 && (ability == ABILITY_OVERCOAT || ability == ABILITY_WIDE_HAT))
        || (GetConfig(CONFIG_POWDER_GRASS) >= GEN_6 && IS_BATTLER_OF_TYPE(battler, TYPE_NEW_NATURE))
