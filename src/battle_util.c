...
static enum MoveCanceler CancelerParalyzed(struct BattleContext *ctx)
{
    if (gBattleMons[ctx->battlerAtk].status1 & STATUS1_PARALYSIS
        && !(B_MAGIC_GUARD == GEN_4 && (IsAbilityAndRecord(ctx->battlerAtk, ctx->abilities[ctx->battlerAtk], ABILITY_MAGIC_GUARD) || IsAbilityAndRecord(ctx->battlerAtk, ctx->abilities[ctx->battlerAtk], ABILITY_FANTASY_BREAKER)))
...
static enum MoveCanceler CancelerThaw(struct BattleContext *ctx)
{
    ...
    {
        if (!(IsMoveEffectRemoveSpeciesType(ctx->currentMove, MOVE_EFFECT_REMOVE_ARG_TYPE, TYPE_NEW_FIRE) && !IS_BATTLER_OF_TYPE(ctx->battlerAtk, TYPE_NEW_FIRE)))
        {
            gBattleMons[ctx->battlerAtk].status1 &= ~STATUS1_FREEZE;
            BattleScriptCall(BattleScript_MoveUsedUnfroze);
...
static enum MoveCanceler CancelerWeatherPrimal(struct BattleContext *ctx)
{
    ...
    if (HasWeatherEffect() && GetMovePower(ctx->currentMove) > 0)
    {
        ...
        if (moveType == TYPE_NEW_FIRE && (gBattleWeather & B_WEATHER_RAIN_PRIMAL) && (GetGenConfig(GEN_CONFIG_POWDER_STATUS_HEAVY_RAIN) >= GEN_7 || !TryActivatePowderStatus(ctx->currentMove)))
        {
...
static enum MoveCanceler CancelerPowderStatus(struct BattleContext *ctx)
{
    ...
    {
        if (!(IsAbilityAndRecord(ctx->battlerAtk, ctx->abilities[ctx->battlerAtk], ABILITY_MAGIC_GUARD) || IsAbilityAndRecord(ctx->battlerAtk, ctx->abilities[ctx->battlerAtk], ABILITY_FANTASY_BREAKER)))
...
static enum MoveCanceler CancelerPriorityBlock(struct BattleContext *ctx)
{
    ...
    if (IsDazzlingAbility(ctx->abilities[ctx->battlerDef]) && !IsAbilityOnField(ABILITY_TIME_JACKER))
    {
...
bool32 CanAbilityAbsorbMove(u32 battlerAtk, u32 battlerDef, enum Ability abilityDef, u32 move, enum Type moveType, enum FunctionCallOption option)
{
    ...
    case ABILITY_VOLT_ABSORB:
        if (moveType == TYPE_NEW_ELECTRIC && GetBattlerMoveTargetType(battlerAtk, move) != MOVE_TARGET_ALL_BATTLERS && gCurrentMove != MOVE_BINARY_CRUSHER)
    ...
    case ABILITY_LIGHTNING_ROD:
        if (B_REDIRECT_ABILITY_IMMUNITY >= GEN_5 && moveType == TYPE_NEW_ELECTRIC && GetBattlerMoveTargetType(battlerAtk, move) != MOVE_TARGET_ALL_BATTLERS && gCurrentMove != MOVE_BINARY_CRUSHER)
        {
            effect = MOVE_ABSORBED_BY_STAT_INCREASE_ABILITY;
            statId = STAT_SPATK;
        }
    ...
    case ABILITY_STORM_DRAIN:
        if (B_REDIRECT_ABILITY_IMMUNITY >= GEN_5 && moveType == TYPE_NEW_WATER)
        {
            effect = MOVE_ABSORBED_BY_STAT_INCREASE_ABILITY;
            statId = STAT_SPATK;
        }
...
bool32 TryFieldEffects(enum FieldEffectCases caseId)
{
    ...
    case FIELD_EFFECT_TRAINER_STATUSES:  // starting field/side/etc statuses with a variable
        ...
        else if (gBattleStruct->startingStatus & STARTING_STATUS_UBW)
        {
            effect = SetStartingFieldStatus(
                        STATUS_FIELD_UBW,
                        B_MSG_UBW_SET,
                        0,
                        &gFieldTimers.terrainTimer);
            gBattleStruct->startingStatus &= ~STARTING_STATUS_UBW;
            isTerrain = TRUE;
        }
        else if (gBattleStruct->startingStatus & STARTING_STATUS_DARKNESS_TERRAIN)
        {
            effect = SetStartingFieldStatus(
                        ...
                        0,
                        &gFieldTimers.terrainTimer);
            gBattleStruct->startingStatus &= ~STARTING_STATUS_DARKNESS_TERRAIN;
            isTerrain = TRUE;
        }
        else if (gBattleStruct->startingStatus & STARTING_STATUS_MIASMA_TERRAIN)
        {
            effect = SetStartingFieldStatus(
                        ...
                        0,
                        &gFieldTimers.terrainTimer);
            gBattleStruct->startingStatus &= ~STARTING_STATUS_MIASMA_TERRAIN;
            isTerrain = TRUE;
        }
...
bool32 CanSetNonVolatileStatus(u32 battlerAtk, u32 battlerDef, enum Ability abilityAtk, enum Ability abilityDef, enum MoveEffect effect, enum FunctionCallOption option)
{
    ...
    case MOVE_EFFECT_PARALYSIS:
        ...
        else if (GetGenConfig(GEN_CONFIG_PARALYZE_ELECTRIC) >= GEN_6 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_ELECTRIC))
        {
            battleScript = BattleScript_NotAffected;
        }
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    if (GetMoveEffect(ctx->chosenMove) == EFFECT_ME_FIRST)
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
    if (IsBattlerTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, STATUS_FIELD_GRASSY_TERRAIN) && moveType == TYPE_NEW_NATURE)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsBattlerTerrainAffected(battlerDef, ctx->abilityDef, ctx->holdEffectDef, STATUS_FIELD_MISTY_TERRAIN) && moveType == TYPE_NEW_DARK)
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
    if (IsBattlerTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, STATUS_FIELD_ELECTRIC_TERRAIN) && moveType == TYPE_NEW_ELECTRIC)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsBattlerTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, STATUS_FIELD_PSYCHIC_TERRAIN) && moveType == TYPE_NEW_REASON)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsBattlerTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, STATUS_FIELD_UBW) && moveType == TYPE_NEW_METAL)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsBattlerTerrainAffected(battlerDef, ctx->abilityAtk, ctx->holdEffectAtk, STATUS_FIELD_UBW) && moveType == TYPE_NEW_MIASMA)
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
    if (IsBattlerTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, STATUS_FIELD_DARKNESS_TERRAIN) && moveType == TYPE_NEW_DARK)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsBattlerTerrainAffected(battlerDef, ctx->abilityAtk, ctx->holdEffectAtk, STATUS_FIELD_DARKNESS_TERRAIN) && moveType == TYPE_NEW_DIVINE)
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
...
static inline u32 CalcAttackStat(struct DamageContext *ctx)
{
    ...
    case ABILITY_DEFEATIST:
        ...
        break;
    case ABILITY_FLASH_FIRE:
        if (moveType == TYPE_NEW_FIRE && gDisableStructs[battlerAtk].flashFireBoosted)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_SWARM:
...
static inline u32 CalcDefenseStat(struct DamageContext *ctx)
{
    ...
    if (B_SANDSTORM_SPDEF_BOOST >= GEN_4 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_EARTH) && IsBattlerWeatherAffected(battlerDef, B_WEATHER_SANDSTORM) && !usesDefStat)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct DamageContext *ctx, uq4_12_t modifier)
{
    ...
    if (ctx->moveType == TYPE_NEW_FIRE && gDisableStructs[ctx->battlerDef].tarShot)
        modifier = uq4_12_multiply(modifier, UQ_4_12(2.0));
    ...
    else if (GetGenConfig(GEN_CONFIG_SHEER_COLD_IMMUNITY) >= GEN_7 && GetMoveEffect(ctx->move) == EFFECT_SHEER_COLD && IS_BATTLER_OF_TYPE(ctx->battlerDef, TYPE_NEW_ICE))
    {
        modifier = UQ_4_12(0.0);
    }
...
bool32 IsAffectedByPowderMove(u32 battler, u32 ability, enum HoldEffect holdEffect)
{
    if ((GetGenConfig(GEN_CONFIG_POWDER_OVERCOAT) >= GEN_6 && ability == ABILITY_OVERCOAT)
        || (GetGenConfig(GEN_CONFIG_POWDER_GRASS) >= GEN_6 && IS_BATTLER_OF_TYPE(battler, TYPE_NEW_NATURE))
