...
static bool8 LoadBattlerSpriteGfx(u32 battler)
{
    if (battler < gBattlersCount)
    {
        if (!IsOnPlayerSide(battler))
        {
            ...
            DecompressTrainerBackPic(TRAINER_BACK_PIC_STEVEN, battler);
...
void CreateBattlerSprite(u32 battler)
{
    if (battler < gBattlersCount)
    {
        ...
        {
            SetMultiuseSpriteTemplateToTrainerBack(TRAINER_BACK_PIC_STEVEN, GetBattlerPosition(0));
            gBattlerSpriteIds[battler] = CreateSprite(&gMultiuseSpriteTemplate, 0x50,
                                                (8 - gTrainerBacksprites[TRAINER_BACK_PIC_STEVEN].coordinates.size) * 4 + 80,
                                                 GetBattlerSpriteSubpriority(0));
            gSprites[gBattlerSpriteIds[battler]].oam.paletteNum = (8 + battler / 2);
            gSprites[gBattlerSpriteIds[battler]].callback = SpriteCallbackDummy;
            gSprites[gBattlerSpriteIds[battler]].data[0] = battler;
