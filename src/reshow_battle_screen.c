...
static bool8 LoadBattlerSpriteGfx(enum BattlerId battler)
{
    if (battler < gBattlersCount)
    {
        ...
        if (!IsOnPlayerSide(battler))
        {
            ...
            DecompressTrainerBackPic(IS_FRLG ? TRAINER_PIC_BACK_OLD_MAN : TRAINER_PIC_BACK_STEVEN, battler);
        else if (!gBattleSpritesDataPtr->battlerData[battler].behindSubstitute)
            BattleLoadMonSpriteGfx(GetBattlerMon(battler), battler);
        else
            BattleLoadSubstituteOrMonSpriteGfx(battler, FALSE);
...
void CreateBattlerSprite(enum BattlerId battler)
{
    if (battler < gBattlersCount)
    {
        ...
        {
            SetMultiuseSpriteTemplateToTrainerBack(IS_FRLG ? TRAINER_PIC_BACK_OLD_MAN : TRAINER_PIC_BACK_STEVEN, position);
            gBattlerSpriteIds[battler] = CreateSprite(&gMultiuseSpriteTemplate, 0x50,
                                                (8 - gTrainerBacksprites[IS_FRLG ? TRAINER_PIC_BACK_OLD_MAN : TRAINER_PIC_BACK_STEVEN].coordinates.size) * 4 + 80,
                                                 GetBattlerSpriteSubpriority(0));
            gSprites[gBattlerSpriteIds[battler]].oam.paletteNum = 14 + battler / 2;
            gSprites[gBattlerSpriteIds[battler]].callback = SpriteCallbackDummy;
            gSprites[gBattlerSpriteIds[battler]].data[0] = battler;
