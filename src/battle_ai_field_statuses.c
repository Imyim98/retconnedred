...
static enum FieldEffectOutcome BenefitsFromElectricTerrain(enum BattlerId battler)
{
    ...
    || HasDamagingMoveOfType(battler, TYPE_NEW_ELECTRIC)))
        return FIELD_EFFECT_POSITIVE;
...
static enum FieldEffectOutcome BenefitsFromGrassyTerrain(enum BattlerId battler)
{
    ...
    if (grounded && HasDamagingMoveOfType(battler, TYPE_NEW_NATURE))
        return FIELD_EFFECT_POSITIVE;
...
static enum FieldEffectOutcome BenefitsFromMistyTerrain(enum BattlerId battler)
{
    ...
    if ((grounded || allyGrounded)
     && (HasDamagingMoveOfType(LEFT_FOE(battler), TYPE_NEW_DARK) || HasDamagingMoveOfType(RIGHT_FOE(battler), TYPE_NEW_DARK)))
        return FIELD_EFFECT_POSITIVE;
...
static enum FieldEffectOutcome BenefitsFromPsychicTerrain(enum BattlerId battler)
{
    ...
    if (grounded || allyGrounded)
    {
        ...
        if (AI_IsAbilityOnSide(LEFT_FOE(battler), ABILITY_GALE_WINGS)
         ...
         || AI_IsAbilityOnSide(LEFT_FOE(battler), ABILITY_TIME_CONTROL)
         || AI_IsAbilityOnSide(LEFT_FOE(battler), ABILITY_WINDY_EGRET))
            return FIELD_EFFECT_POSITIVE;
    }
    ...
    if (AI_IsAbilityOnSide(battler, ABILITY_GALE_WINGS)
     ...
     || AI_IsAbilityOnSide(battler, ABILITY_TIME_CONTROL)
     || AI_IsAbilityOnSide(battler, ABILITY_WINDY_EGRET))
        return FIELD_EFFECT_NEGATIVE;
...
static enum FieldEffectOutcome BenefitsFromUBW(enum BattlerId battler) 
{
    ...
    return FIELD_EFFECT_NEUTRAL;
    // need to address the benefit if using slicing moves
}
