...
static bool32 FindMonThatAbsorbsOpponentsMove(u32 battler)
{
    ...
    if (incomingType == TYPE_NEW_FIRE)
    {
        absorbingTypeAbilities[numAbsorbingAbilities++] = ABILITY_FLASH_FIRE;
    }
    else if (incomingType == TYPE_NEW_WATER || (isOpposingBattlerChargingOrInvulnerable && incomingType == TYPE_NEW_WATER))
    {
    ...
    else if (incomingType == TYPE_NEW_ELECTRIC || (isOpposingBattlerChargingOrInvulnerable && incomingType == TYPE_NEW_ELECTRIC))
    {
    ...
    else if (incomingType == TYPE_NEW_NATURE || (isOpposingBattlerChargingOrInvulnerable && incomingType == TYPE_NEW_NATURE))
    {
        absorbingTypeAbilities[numAbsorbingAbilities++] = ABILITY_SAP_SIPPER;
    }
    else if (incomingType == TYPE_NEW_EARTH || (isOpposingBattlerChargingOrInvulnerable && incomingType == TYPE_NEW_EARTH))
    {
...
bool32 IsSwitchinGrounded(enum HoldEffect heldItemEffect, enum Ability ability, enum Type type1, enum Type type2)
{
    ...
    if (type1 == TYPE_NEW_FLYING || type2 == TYPE_NEW_FLYING || ability == ABILITY_LEVITATE
    ...
    if (HasWeatherEffect())
    {
        ...
        if (holdEffect != HOLD_EFFECT_SAFETY_GOGGLES && ability != ABILITY_MAGIC_GUARD && ability != ABILITY_FANTASY_BREAKER && ability != ABILITY_OVERCOAT)
        {
             ...
             && ability != ABILITY_SNOW_CLOAK && ability != ABILITY_ICE_BODY)
            {
                ...
                && (gAiLogicData->switchinCandidate.battleMon.types[0] != TYPE_NEW_EARTH && gAiLogicData->switchinCandidate.battleMon.types[1] != TYPE_NEW_EARTH
//                && gAiLogicData->switchinCandidate.battleMon.types[0] != TYPE_ROCK && gAiLogicData->switchinCandidate.battleMon.types[1] != TYPE_ROCK
                && gAiLogicData->switchinCandidate.battleMon.types[0] != TYPE_NEW_METAL && gAiLogicData->switchinCandidate.battleMon.types[1] != TYPE_NEW_METAL
...
static u32 GetSwitchinStatusDamage(u32 battler)
{
    ...
    if ((status != 0) && !(gAiLogicData->switchinCandidate.battleMon.ability != ABILITY_MAGIC_GUARD || gAiLogicData->switchinCandidate.battleMon.ability != ABILITY_FANTASY_BREAKER))
    {
    ...
    if (tSpikesLayers != 0 && (defType1 != TYPE_NEW_MIASMA && defType2 != TYPE_NEW_MIASMA
...
static bool32 AI_CanSwitchinAbilityTrapOpponent(enum Ability ability, u32 opposingBattler)
{
    ...
    else if (ability == ABILITY_SHADOW_TAG)
    {
        ...
        return TRUE;
    else if (ability == ABILITY_MAGNET_PULL && IS_BATTLER_OF_TYPE(opposingBattler, TYPE_NEW_METAL))
        return TRUE;
    else
        return FALSE;
}
