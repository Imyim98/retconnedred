...
static u32 GetSwitchinHazardsDamage(u32 battler)
{
        ...
        !((gFieldStatuses & STATUS_FIELD_MAGIC_ROOM) || ability == ABILITY_KLUTZ)))
    {
        ...
        if (IsHazardOnSide(side, HAZARDS_TOXIC_SPIKES) && (!IS_BATTLER_ANY_TYPE(battler, TYPE_NEW_MIASMA, TYPE_NEW_METAL)
...
static u32 GetSwitchinRecurringHealing(u32 battler)
{
    ...
    {
        if (holdEffect == HOLD_EFFECT_BLACK_SLUDGE && IS_BATTLER_OF_TYPE(battler, TYPE_NEW_MIASMA))
        {
...
static u32 GetSwitchinStatusDamage(u32 battler)
{
    ...
    if ((status != 0) && ability != ABILITY_MAGIC_GUARD && ability != ABILITY_FANTASY_BREAKER)
    {
    ...
    if (tSpikesLayers != 0 && (IS_BATTLER_OF_TYPE(battler, TYPE_NEW_MIASMA)
