...
// the following checks apply to any target (including user)
    ...
    // other
        ...
        case EFFECT_SHEER_COLD:
            if (GetConfig(CONFIG_SHEER_COLD_IMMUNITY) >= GEN_7 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_ICE))
                RETURN_SCORE_MINUS(20);
            // fallthrough
        case EFFECT_OHKO:
        ...
        case EFFECT_LASER_FOCUS:
            if (gBattleMons[battlerDef].volatiles.laserFocus)
                ADJUST_SCORE(-10);
            else if (aiData->abilities[battlerDef] == ABILITY_SHELL_ARMOR || aiData->abilities[battlerDef] == ABILITY_BATTLE_ARMOR  || aiData->abilities[battlerDef] == ABILITY_GUARD_ARMOR)
                ADJUST_SCORE(-8);
            break;
        case EFFECT_SKETCH:
            if (aiData->lastUsedMove[battlerDef] == MOVE_NONE)
        ...
        case EFFECT_BESTOW:
            if (aiData->holdEffects[battlerAtk] == HOLD_EFFECT_NONE
              ...
              || aiData->abilities[battlerAtk] == ABILITY_COLLECTOR
              || DoesSubstituteBlockMove(battlerAtk, battlerDef, move))
                ADJUST_SCORE(-10);
            break;
        case EFFECT_WISH:
...
static s32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, enum Move move, struct AiLogicData *aiData)
{
    ...
    case EFFECT_LUCKY_CHANT:
        ...
        break;
    case EFFECT_MAGNET_RISE:
        if (AI_IsBattlerGrounded(battlerAtk) && HasDamagingMoveOfType(battlerDef, TYPE_NEW_EARTH)
    ...
    case EFFECT_STEAL_ITEM:
        {
            ...
            if (canSteal && aiData->items[battlerAtk] == ITEM_NONE
             ...
             && !(aiData->abilities[battlerDef] == ABILITY_STICKY_HOLD || aiData->abilities[battlerDef] == ABILITY_COLLECTOR))
            {
                switch (aiData->holdEffects[battlerDef])
                {
    ...
    for (u32 effectId = 0; effectId < additionalEffectCount; effectId++)
    {
        ...
        else // consider move effects that hinder the target
        {
            ...
            case MOVE_EFFECT_BUG_BITE:   // And pluck
                if (gBattleMons[battlerDef].volatiles.substitute || aiData->abilities[battlerDef] == ABILITY_STICKY_HOLD || aiData->abilities[battlerDef] == ABILITY_COLLECTOR)
                    break;
