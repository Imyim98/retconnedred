...
static s32 AI_CheckBadMove(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    if (((GetMoveType(move) == TYPE_NEW_FIRE && GetMovePower(move) != 0) || CanBurnHitThaw(move)) && effectiveness < UQ_4_12(2.0) && (gBattleMons[battlerDef].status1 & STATUS1_ICY_ANY))
    {
        ...
        for (i = 0; i < MAX_MON_MOVES; i++)
        {
            ...
            if (GetMoveType(aiMove) != TYPE_NEW_FIRE && !CanBurnHitThaw(aiMove) && GetMovePower(gBattleMons[battlerAtk].moves[i]) != 0)
            {
                ADJUST_SCORE(-1);
                break;
            }
    ...
    if (!(moveTarget & MOVE_TARGET_USER))
    {
        ...
        case ABILITY_MAGIC_GUARD:
            ...
            case EFFECT_LEECH_SEED:
                ...
                break;
            case EFFECT_CURSE:
                if (IS_BATTLER_OF_TYPE(battlerAtk, TYPE_NEW_NETHER)) // Don't use Curse if you're a ghost type vs a Magic Guard user, they'll take no damage.
                    ADJUST_SCORE(-5);
                break;
            default:
                break;
        ...
        case ABILITY_FLOWER_VEIL:
            if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_NATURE) && (IsNonVolatileStatusMove(move)))
                RETURN_SCORE_MINUS(10);
            break;
        ...
        if (hasTwoOpponents)
        {
            ...
            case ABILITY_LIGHTNING_ROD:
                if (moveType == TYPE_NEW_ELECTRIC && !IsMoveRedirectionPrevented(battlerAtk, move, aiData->abilities[battlerAtk]))
                    RETURN_SCORE_MINUS(20);
                break;
            case ABILITY_STORM_DRAIN:
                if (moveType == TYPE_NEW_WATER && !IsMoveRedirectionPrevented(battlerAtk, move, aiData->abilities[battlerAtk]))
                    RETURN_SCORE_MINUS(20);
                break;
            case ABILITY_MAGIC_BOUNCE:
            ...
            case ABILITY_SWEET_VEIL:
                ...
                break;
            case ABILITY_FLOWER_VEIL:
                if ((IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_NATURE)) && (IsNonVolatileStatusMove(move) || IsStatLoweringEffect(moveEffect)))
                    RETURN_SCORE_MINUS(10);
                break;
            case ABILITY_AROMA_VEIL:
                if (IsAromaVeilProtectedEffect(moveEffect))
        ...
        if (GetGenConfig(GEN_CONFIG_PRANKSTER_DARK_TYPES) >= GEN_7 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_DARK)
...
// the following checks apply to any target (including user)
    ...
    // stat lowering effects
        ...
        case EFFECT_EVASION_DOWN_2:
            if (!CanLowerStat(battlerAtk, battlerDef, aiData, STAT_EVASION))
                ADJUST_SCORE(-10);
    ...
    // other
        ...
        case EFFECT_SHEER_COLD:
            if (GetGenConfig(GEN_CONFIG_SHEER_COLD_IMMUNITY) >= GEN_7 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_ICE))
                RETURN_SCORE_MINUS(20);
            // fallthrough
        case EFFECT_OHKO:
        ...
        case EFFECT_FORESIGHT:
            if (gBattleMons[battlerDef].volatiles.foresight)
              ...
              || !(IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_NETHER))
              || DoesPartnerHaveSameMoveEffect(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
                ADJUST_SCORE(-9);
            break;
        ...
        case EFFECT_RECOIL_IF_MISS:
            if (!(aiData->abilities[battlerAtk] == ABILITY_MAGIC_GUARD || aiData->abilities[battlerAtk] == ABILITY_FANTASY_BREAKER) && gAiLogicData->moveAccuracy[battlerAtk][battlerDef][gAiThinkingStruct->movesetIndex] < 75
        ...
        case EFFECT_MIRACLE_EYE:
            ...
            if (gBattleMons[battlerDef].statStages[STAT_EVASION] <= 4
              || !(IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_DARK))
              || DoesPartnerHaveSameMoveEffect(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
                ADJUST_SCORE(-9);
            break;
        ...
        case EFFECT_GRAVITY:
            if ((gFieldStatuses & STATUS_FIELD_GRAVITY
              && !IS_BATTLER_OF_TYPE(battlerAtk, TYPE_NEW_FLYING)
        ...
        case EFFECT_EMBARGO:
            ...
            break;
        case EFFECT_POWDER:
            if (!HasMoveWithType(battlerDef, TYPE_NEW_FIRE)
              || PartnerMoveIsSameAsAttacker(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
                ADJUST_SCORE(-10);
            break;
        ...
        case EFFECT_ELECTRIFY:
            if (AI_IsSlower(battlerAtk, battlerDef, move, predictedMoveSpeedCheck, CONSIDER_PRIORITY)
              //|| GetMoveTypeSpecial(battlerDef, predictedMove) == TYPE_NEW_ELECTRIC // Move will already be electric type
              || PartnerMoveIsSameAsAttacker(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
                ADJUST_SCORE(-10);
            break;
        ...
        case EFFECT_SKY_DROP:
            if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_FLYING))
                ADJUST_SCORE(-10);
            if (BattlerWillFaintFromWeather(battlerAtk, aiData->abilities[battlerAtk])
        ...
        if (gBattleMons[battlerAtkPartner].volatiles.dragonCheer
         ...
         || !HasDamagingMove(battlerAtkPartner))
            ADJUST_SCORE(-5);
              ...
              || IS_BATTLER_OF_TYPE(battlerAtkPartner, TYPE_NEW_HEART)
    ...
    if (IsTargetingPartner(battlerAtk, battlerDef))
    {
        ...
        if (!partnerProtecting && moveTarget != MOVE_TARGET_BOTH && !DoesBattlerIgnoreAbilityChecks(battlerAtk, aiData->abilities[battlerAtk], move))
        {
            ...
            case ABILITY_WATER_COMPACTION:
                if (moveType == TYPE_NEW_WATER && isFriendlyFireOK
            ...
            case ABILITY_SAP_SIPPER:
                ...
                break;
            case ABILITY_JUSTIFIED:
                if (moveType == TYPE_NEW_DARK && isFriendlyFireOK
                    && !IsBattleMoveStatus(move)
                    && ShouldTriggerAbility(battlerAtk, battlerAtkPartner, atkPartnerAbility))
                {
                    if (moveTarget == MOVE_TARGET_FOES_AND_ALLY)
        ...
        if (!partnerProtecting)
        {
            ...
            case EFFECT_SOAK:
                if (atkPartnerAbility == ABILITY_WONDER_GUARD
                 && !IS_BATTLER_OF_TYPE(battlerAtkPartner, TYPE_NEW_WATER)
...
static s32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move, struct AiLogicData *aiData)
{
    ...
    case EFFECT_ROAR:
        if ((gMovesInfo[move].soundMove && aiData->abilities[battlerDef] == ABILITY_SOUNDPROOF)
          || aiData->abilities[battlerDef] == ABILITY_SUCTION_CUPS || aiData->abilities[battlerDef] == ABILITY_FANTASY_BREAKER)
            break;
        else if (GetActiveGimmick(battlerDef) == GIMMICK_DYNAMAX)
            break;
        score += AI_TryToClearStats(battlerAtk, battlerDef, moveTargetsBothOpponents);
    ...
    case EFFECT_POPULATION_BOMB:
        if (AI_MoveMakesContact(aiData->abilities[battlerAtk], aiData->holdEffects[battlerAtk], move)
          && !(aiData->abilities[battlerAtk] == ABILITY_MAGIC_GUARD || aiData->abilities[battlerAtk] == ABILITY_FANTASY_BREAKER)
    ...
    case EFFECT_FORESIGHT:
        if (aiData->abilities[battlerAtk] == ABILITY_SCRAPPY || aiData->abilities[battlerAtk] == ABILITY_MINDS_EYE)
         ...
         || (IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_NETHER)
         && (HasMoveWithType(battlerAtk, TYPE_NEW_ILLUSION)
         || HasMoveWithType(battlerAtk, TYPE_NEW_DREAM))))
            ADJUST_SCORE(DECENT_EFFECT);
        break;
    case EFFECT_MIRACLE_EYE:
        if (gBattleMons[battlerDef].statStages[STAT_EVASION] > DEFAULT_STAT_STAGE
          || (IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_DARK) && (HasMoveWithType(battlerAtk, TYPE_NEW_REASON))))
            ADJUST_SCORE(DECENT_EFFECT);
        break;
    case EFFECT_PERISH_SONG:
        if (IsBattlerTrapped(battlerAtk, battlerDef))
    ...
    case EFFECT_FOLLOW_ME:
        ...
        break;
    case EFFECT_CHARGE:
        if (HasDamagingMoveOfType(battlerAtk, TYPE_NEW_ELECTRIC))
            ADJUST_SCORE(DECENT_EFFECT);
    ...
    case EFFECT_BESTOW:
        ...
        case HOLD_EFFECT_BLACK_SLUDGE:
            if (!IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_MIASMA) && !(aiData->abilities[battlerDef] == ABILITY_MAGIC_GUARD || aiData->abilities[battlerDef] == ABILITY_FANTASY_BREAKER))
                ADJUST_SCORE(DECENT_EFFECT);
            break;
        ...
        default:
            if (GetMoveEffect(move) != EFFECT_BESTOW && aiData->items[battlerAtk] == ITEM_NONE && aiData->items[battlerDef] != ITEM_NONE)
            {
                ...
                case HOLD_EFFECT_FLAME_ORB:
                    ...
                    break;
                case HOLD_EFFECT_BLACK_SLUDGE:
                    if (IS_BATTLER_OF_TYPE(battlerAtk, TYPE_NEW_MIASMA) || (aiData->abilities[battlerAtk] == ABILITY_MAGIC_GUARD || aiData->abilities[battlerAtk] == ABILITY_FANTASY_BREAKER))
                        ADJUST_SCORE(DECENT_EFFECT);
                    break;
                case HOLD_EFFECT_IRON_BALL:
    ...
    case EFFECT_SNATCH:
        ...
        break;
    case EFFECT_MUD_SPORT:
        if (!HasMoveWithType(battlerAtk, TYPE_NEW_ELECTRIC) && HasMoveWithType(battlerDef, TYPE_NEW_ELECTRIC))
            ADJUST_SCORE(WEAK_EFFECT);
        break;
    case EFFECT_WATER_SPORT:
        if (!HasMoveWithType(battlerAtk, TYPE_NEW_FIRE) && (HasMoveWithType(battlerDef, TYPE_NEW_FIRE)))
            ADJUST_SCORE(WEAK_EFFECT);
        break;
    case EFFECT_TICKLE:
        ADJUST_SCORE(IncreaseStatDownScore(battlerAtk, battlerDef, STAT_ATK));
    ...
    case EFFECT_PSYCHIC_TERRAIN:
        ...
        break;
    case EFFECT_UBW:
        if (ShouldSetFieldStatus(battlerAtk, STATUS_FIELD_UBW))
        {
            ADJUST_SCORE(GOOD_EFFECT);
            if (aiData->holdEffects[battlerAtk] == HOLD_EFFECT_TERRAIN_EXTENDER || HasBattlerSideMoveWithEffect(battlerAtk, EFFECT_TERRAIN_PULSE))
                ADJUST_SCORE(WEAK_EFFECT);
        ...
        break;
    case EFFECT_DARKNESS_TERRAIN:
        if (ShouldSetFieldStatus(battlerAtk, STATUS_FIELD_DARKNESS_TERRAIN))
        {
            ADJUST_SCORE(GOOD_EFFECT);
            if (aiData->holdEffects[battlerAtk] == HOLD_EFFECT_TERRAIN_EXTENDER || HasBattlerSideMoveWithEffect(battlerAtk, EFFECT_TERRAIN_PULSE))
                ADJUST_SCORE(WEAK_EFFECT);
    ...
    case EFFECT_EMBARGO:
        ...
        break;
    case EFFECT_POWDER:
        if (predictedMove != MOVE_NONE && !IsBattleMoveStatus(predictedMove) && predictedType == TYPE_NEW_FIRE)
            ADJUST_SCORE(DECENT_EFFECT);
        break;
    case EFFECT_TELEKINESIS:
    ...
    case EFFECT_HEAL_BLOCK:
        if (AI_IsFaster(battlerAtk, battlerDef, move, predictedMoveSpeedCheck, CONSIDER_PRIORITY) && predictedMove != MOVE_NONE && IsHealingMove(predictedMove))
          ...
          || (aiData->holdEffects[battlerDef] == HOLD_EFFECT_BLACK_SLUDGE && IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_MIASMA)))
            ADJUST_SCORE(DECENT_EFFECT);
        break;
    case EFFECT_SOAK:
        if (HasMoveWithType(battlerAtk, TYPE_NEW_ELECTRIC) || HasMoveWithType(battlerAtk, TYPE_NEW_NATURE)
        || HasMoveWithType(BATTLE_PARTNER(battlerAtk), TYPE_NEW_ELECTRIC) || HasMoveWithType(BATTLE_PARTNER(battlerAtk), TYPE_NEW_NATURE)
        || (HasBattlerSideMoveWithEffect(battlerAtk, EFFECT_SUPER_EFFECTIVE_ON_ARG) && GetMoveArgType(move) == TYPE_NEW_WATER) )
    ...
    case EFFECT_LUCKY_CHANT:
        ...
        break;
    case EFFECT_MAGNET_RISE:
        if (AI_IsBattlerGrounded(battlerAtk) && HasDamagingMoveOfType(battlerDef, TYPE_NEW_ELECTRIC)
          && !(effectiveness == UQ_4_12(0.0))) // Doesn't resist ground move
        {
           ...
           {
                if (predictedType == TYPE_NEW_EARTH)
                    ADJUST_SCORE(GOOD_EFFECT); // Cause the enemy's move to fail
                break;
            }
            else // Opponent Goes First
            {
                if (HasDamagingMoveOfType(battlerDef, TYPE_NEW_EARTH))
                    ADJUST_SCORE(DECENT_EFFECT);
                break;
            }
        }
