...
bool32 AI_IsDamagedByRecoil(u32 battler)
{
    ...
    if (ability == ABILITY_MAGIC_GUARD || ability == ABILITY_ROCK_HEAD || ability == ABILITY_ROCK_HEAD)
        return FALSE;
    return TRUE;
}
...
bool32 ShouldTryOHKO(u32 battlerAtk, u32 battlerDef, enum Ability atkAbility, enum Ability defAbility, u32 move)
{
    ...
    else    // test the odds
    {
        ...
        if (B_SHEER_COLD_ACC >= GEN_7 && GetMoveEffect(move) == EFFECT_SHEER_COLD && !IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_NEW_ICE))
            odds -= 10;
...
u32 GetBattlerSecondaryDamage(u32 battlerId)
{
    ...
    if (gAiLogicData->abilities[battlerId] == ABILITY_MAGIC_GUARD || gAiLogicData->abilities[battlerId] == ABILITY_FANTASY_BREAKER)
        return FALSE;
...
bool32 ShouldTriggerAbility(u32 battlerAtk, u32 battlerDef, enum Ability ability)
{
    if (IsTargetingPartner(battlerAtk, battlerDef))
    {
        ...
        case ABILITY_FLASH_FIRE:
            return (HasMoveWithType(battlerDef, TYPE_NEW_FIRE) && !gDisableStructs[battlerDef].flashFireBoosted);
