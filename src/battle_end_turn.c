...
static bool32 HandleEndTurnWeatherDamage(u32 battler)
{
    ...
    case BATTLE_WEATHER_SUN_PRIMAL:
        if (ability == ABILITY_DRY_SKIN || ability == ABILITY_SOLAR_POWER || ability == ABILITY_AUTUMN_GODDESSES)
        {
            if (AbilityBattleEffects(ABILITYEFFECT_ENDTURN, battler, ability, 0, MOVE_NONE))
                effect = TRUE;
        }
...
static bool32 HandleEndTurnBurn(u32 battler)
{
    ...
    if (gBattleMons[battler].status1 & STATUS1_BURN
     && IsBattlerAlive(battler)
     && !(IsAbilityAndRecord(battler, ability, ABILITY_MAGIC_GUARD) || IsAbilityAndRecord(battler, ability, ABILITY_PURE_WHITE)))
    {
...
static bool32 HandleEndTurnWrap(u32 battler)
{
    ...
    {
        if (gDisableStructs[battler].wrapTurns != 0 && ability != ABILITY_PURE_WHITE)
        {
...
static bool32 HandleEndTurnPerishSong(u32 battler)
{
    ...
    if (IsBattlerAlive(battler) && gBattleMons[battler].volatiles.perishSong && ability != ABILITY_PURE_WHITE)
    {
