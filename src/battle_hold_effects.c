...
static enum ItemEffect TryTerrainSeeds(enum BattlerId battler, enum Item item)
{
	...
	case HOLD_EFFECT_PARAM_UBW:
		effect = TryHandleSeed(battler, STATUS_FIELD_UBW, STAT_ATK);
		break;
	case HOLD_EFFECT_PARAM_DARKNESS_TERRAIN:
		effect = TryHandleSeed(battler, STATUS_FIELD_DARKNESS_TERRAIN, STAT_SPATK);
		break;
	case HOLD_EFFECT_PARAM_MIASMA_TERRAIN:
		effect = TryHandleSeed(battler, STATUS_FIELD_MIASMA_TERRAIN, STAT_ATK);
		break;
    }
