...
bool32 IsMovePowderBlocked(struct BattleContext *ctx)
{
    ...
    if (IsPowderMove(ctx->currentMove) && (ctx->battlerAtk != ctx->battlerDef))
    {
        if (GetGenConfig(GEN_CONFIG_POWDER_GRASS) >= GEN_6
         && (IS_BATTLER_OF_TYPE(ctx->battlerDef, TYPE_NEW_NATURE) || ctx->ability[ctx->battlerDef] == ABILITY_OVERCOAT))
        {
...
static void Cmd_attackcanceler(void)
{
    ...
    else if (isBounceable && !gBattleStruct->bouncedMoveIsUsed)
    {
        ...
        if (ctx.ability[ctx.battlerDef] == ABILITY_MAGIC_BOUNCE || ctx.ability[ctx.battlerDef] == ABILITY_FANTASY_BREAKER)
        {
    ...
    if (gSpecialStatuses[gBattlerTarget].abilityRedirected)
    {
     ...
     && (ctx.moveEffect != EFFECT_CURSE || IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_NEW_NETHER))
     && (!gBattleMoveEffects[ctx.moveEffect].twoTurnEffect || (gBattleMons[gBattlerAttacker].volatiles.multipleTurns))
