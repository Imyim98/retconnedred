...
void (*const gBattleScriptingCommandsTable[])(void) =
{
    ...
    Cmd_pickup,                                  //0xE5
    Cmd_tryreducefoespartyhptozero,              // Cmd_unused_0xE6,                                 //0xE6
    Cmd_ishplessthanquarter,                     // Cmd_unused_0xE7,                             //0xE7
...
bool32 IsMovePowderBlocked(struct BattleContext *ctx)
{
    ...
    if (IsPowderMove(ctx->currentMove) && (ctx->battlerAtk != ctx->battlerDef))
    {
        if (GetGenConfig(GEN_CONFIG_POWDER_GRASS) >= GEN_6
         && (IS_BATTLER_OF_TYPE(ctx->battlerDef, TYPE_NEW_NATURE) || ctx->abilities[ctx->battlerDef] == ABILITY_OVERCOAT))
        {
...
static void Cmd_attackcanceler(void)
{
    ...
    else if (isBounceable && !gBattleStruct->bouncedMoveIsUsed)
    {
        ...
        if (ctx.abilities[ctx.battlerDef] == ABILITY_MAGIC_BOUNCE || ctx.abilities[ctx.battlerDef] == ABILITY_FANTASY_BREAKER)
        {
    ...
    if (gSpecialStatuses[gBattlerTarget].abilityRedirected)
    {
     ...
     && (moveEffect != EFFECT_CURSE || IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_NEW_NETHER))
     && (!gBattleMoveEffects[moveEffect].twoTurnEffect || (gBattleMons[gBattlerAttacker].volatiles.multipleTurns))
...
static inline bool32 TryStrongWindsWeakenAttack(u32 battlerDef, u32 moveType)
{
    if (gBattleWeather & B_WEATHER_STRONG_WINDS && HasWeatherEffect())
    {
        if (GetMoveCategory(gCurrentMove) != DAMAGE_CATEGORY_STATUS
         ...
         && gTypeEffectivenessTable[moveType][TYPE_NEW_FLYING] >= UQ_4_12(2.0)
...
void SetMoveEffect(u32 battler, u32 effectBattler, enum MoveEffect moveEffect, const u8 *battleScript, enum SetMoveEffectFlags effectFlags)
{
    ...
    case MOVE_EFFECT_FLINCH:
		...
		else if (battlerAbility == ABILITY_PURE_WHITE)
		{
			...
			if (primary || certain)
			{
				...
				gBattlerAbility = gEffectBattler;
				RecordAbilityBattle(gEffectBattler, ABILITY_PURE_WHITE);
				gBattlescriptCurrInstr = BattleScript_FlinchPrevention;
			}
			else
			{
    ...
    case MOVE_EFFECT_PAYDAY:
        ...
        if (IsOnPlayerSide(gBattlerAttacker) && gSpecialStatuses[gBattlerAttacker].parentalBondState!= PARENTAL_BOND_2ND_HIT)
        {
            ...
            if (gCurrentMove == MOVE_TAX_COLLECTION)
                gPaydayMoney += (gBattleMons[gBattlerAttacker].level * 50);
            else
    ...
    case MOVE_EFFECT_LEECH_SEED:
        if (!IS_BATTLER_OF_TYPE(gBattlerTarget, TYPE_NEW_NATURE) && !gBattleMons[gBattlerTarget].volatiles.leechSeed)
        {
...
static void Cmd_moveend(void)
{
    ...
    do
    {
        ...
        case MOVEEND_CLEAR_BITS: // Clear/Set bits for things like using a move for all targets and all hits.
            ...
            if (B_CHARGE >= GEN_9 && moveType == TYPE_NEW_ELECTRIC && (IsBattlerTurnDamaged(gBattlerTarget) || gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT))
...
void TryHazardsOnSwitchIn(u32 battler, u32 side, enum Hazards hazardType)
{
    ...
    {
        enum Ability ability = GetBattlerAbility(battler);
        if (!(GetBattlerAbility(battler) == ABILITY_MAGIC_GUARD || GetBattlerAbility(battler) == ABILITY_FANTASY_BREAKER)
...
static void Cmd_setfocusenergy(void)
{
    ...
    else if (effect == EFFECT_DRAGON_CHEER && !IS_BATTLER_OF_TYPE(battler, TYPE_NEW_HEART))
    {
...
static void Cmd_settypetoenvironment(void)
{
    ...
    {
    case STATUS_FIELD_ELECTRIC_TERRAIN:
        environmentType = TYPE_NEW_ELECTRIC;
        break;
    case STATUS_FIELD_GRASSY_TERRAIN:
        environmentType = TYPE_NEW_NATURE;
        break;
    case STATUS_FIELD_MISTY_TERRAIN:
        environmentType = TYPE_NEW_DIVINE;
        break;
    case STATUS_FIELD_PSYCHIC_TERRAIN:
        environmentType = TYPE_NEW_REASON;
        break;
    case STATUS_FIELD_UBW:
    ...
    else
    {
        ...
        else
        {
            ...
            case BALL_SAFARI:
                ...
                break;
            case BALL_NET:
                if (IS_BATTLER_ANY_TYPE(gBattlerTarget, TYPE_NEW_WATER, TYPE_NEW_BEAST))
...
void BS_TransformDataExecutionDecade(void)
{
    ...
    {
        ctx.move = sTransformationDCDMockUpMoves[i];
        ctx.moveType = CheckDynamicMoveType(GetBattlerMon(ctx.battlerAtk), ctx.move, ctx.battlerAtk, MON_IN_BATTLE);
        calcDamage = CalculateMoveDamage(&ctx);
    ...
    // somehow didn't find one that does more than 0
    if (moveUsed == MOVE_NONE)
        moveUsed = sTransformationDCDMockUpMoves[Random() % ARRAY_COUNT(sTransformationDCDMockUpMoves)];
    ...
    else
    {
        ...
        else
        {
            ...
            if (gCurrentMove == MOVE_FLUFFICATION)
            {
                ...
                ivDef = gBattleMons[gBattlerTarget].defenseIV;
                ivSpAtk = gBattleMons[gBattlerTarget].speedIV;
                ...
                gBattleMons[gBattlerTarget].attack = calcAtk;
                gBattleMons[gBattlerTarget].defense = calcDef;
                ...
                {
                    gBattleMons[gBattlerTarget].pp[j] = 0;
                }
            ...
            else
            {
                ...
                {
                    if (gMovesInfo[gBattleMons[gBattlerAttacker].moves[j]].pp < 10)
                        gBattleMons[gBattlerAttacker].pp[j] = gMovesInfo[gBattleMons[gBattlerAttacker].moves[j]].pp;
                    else
                        gBattleMons[gBattlerAttacker].pp[j] = 10;
                }
